...
def vcatasks(name,parameters,basis,cell,lattice,terms,weiss=(),baths=(),jobs=()):
    ...
    if 'EB' in jobs:
        vca.register(VCA.EB(name='EB',path=square_gxm(reciprocals=cell.reciprocals,nk=100),mu=0.0,emin=3.0,emax=15.0,eta=0.05,ne=401,savedata=False, plot=True,returndata=False,run=VCA.VCAEB))
    if 'DOS' in jobs:
        vca.register(
            DOS(name='DOS', ne=401, BZ=KSpace(reciprocals=cell.reciprocals, nk=100),mu=0.0, emin=3.0, emax=15.0,eta=0.05,savedata=False, plot=True,returndata=False,run=VCA.VCADOS))
    vca.summary()
...
if __name__=='__main__':
    ...
    parameters['t1'] = 0.160
    parameters['t2'] = 0.161
    parameters['t12'] = 0.00166

    parameters['U0'] = 2.20
    parameters['U1'] = 2.20
    parameters['U2'] = 2.25
    parameters['U01'] = [0.0,2.19,0.0,0.0]
    parameters['U02'] = [0.0,2.40,0.0,0.0]
    parameters['U12'] = [0.0,2.23,0.0,0.0]
